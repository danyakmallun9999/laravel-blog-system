                ...
                <div class="p-6 md:p-8 text-slate-700">
                    @if ($post->image)
                        <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}"
                            class="w-full max-h-[400px] object-cover rounded-lg mb-6 shadow-md">
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6 pb-6 border-b border-slate-200">
                        ...
                        <div>
                            ...
                            <span
                                class="text-sm text-slate-800 font-semibold">{{ $post->created_at->format('F j, Y, g:i a') }}</span>
                        </div>
                        <div>
                            <span class="text-xs font-medium text-slate-500 block mb-1">Last Updated:</span>
                            <span
                                class="text-sm text-slate-800 font-semibold">{{ $post->updated_at->format('F j, Y, g:i a') }}</span>
                        </div>
