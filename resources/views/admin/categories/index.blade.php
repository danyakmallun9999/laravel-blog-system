            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                ...
                                @foreach ($categories as $category)
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap"><span
                                                class="font-semibold text-slate-800">{{ $category->name }}</span></td>
                                        <td class="px-6 py-4 whitespace-nowrap"><span
                                                class="text-sm text-slate-600 font-mono">{{ $category->slug }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap"><span
                        ...
                        @foreach ($categories as $category)
                            <div x-data="{ show: false }" class="p-4">
                                ...
                                <p class="text-xs text-slate-500 mb-4">
                                    Slug: <span
                                        class="font-mono bg-slate-100 px-2 py-0.5 rounded">{{ $category->slug }}</span>
                                </p>
                                <div class="flex justify-end items-center">
                                    <a href="{{ route('admin.categories.edit', $category) }}"
                                        class="px-3 py-1 text-sm font-medium text-blue-600 hover:text-blue-800 transition">
                                        Edit
                                    </a>
                                    <div class="border-l border-slate-300 h-4 mx-2"></div>
                                    {{-- Penambahan Modal untuk Mobile --}}
                                    <form x-ref="deleteForm"
                                ...
                                <x-confirm-deletion-modal title="Hapus Kategori"
                                    message="Apakah Anda yakin ingin menghapus kategori '{{ $category->name }}'? Tindakan ini tidak dapat diurungkan.">
                                    <x-slot:footer>
                                        <button type="button" @click="$refs.deleteForm.submit()"
                                            class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto">
                                            Ya, Hapus
                ...
                @else
                    <div class="text-center py-16 px-6">
                        ...
                        <p class="text-slate-500 mb-6">Get started by creating your first category to organize your blog
                            posts.</p>
                        <a href="{{ route('admin.categories.create') }}"
