...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-2xl text-slate-800 leading-tight"> {{-- Disesuaikan ke text-2xl --}}
                {{ __('Add New Category') }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-2">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-200 bg-slate-50">
                    <div class="flex items-center">
                        <i class="fas fa-folder-plus text-slate-600 mr-3 text-xl"></i> {{-- Icon color disesuaikan ke slate --}}
                        <div>
                            <h3 class="text-lg font-semibold text-slate-800">Category Information</h3>
                            <p class="text-sm text-slate-600">Enter details for the new category</p> {{-- Deskripsi tambahan --}}
                        </div>
                    </div>
                </div>

                <div class="p-6 md:p-8"> {{-- Padding disamakan --}}
                    <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-6">
                        ...
                        <div class="flex flex-col sm:flex-row items-center justify-between pt-6 mt-4 border-t border-slate-200 space-y-3 sm:space-y-0">
                            ...
                            <button type="submit"
                                    class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white font-semibold text-xs uppercase tracking-widest rounded-lg shadow-sm hover:shadow-md transition-all duration-200 group active:bg-slate-800 focus:outline-none focus:border-slate-800 focus:ring ring-slate-300"> {{-- Style tombol save disamakan --}}
            ...
            <div class="mt-6 bg-sky-50 rounded-xl border border-sky-300 p-6 shadow-sm"> {{-- Warna diubah ke sky untuk info --}}
                <div class="flex items-start">
                    <div class="shrink-0">
                        <i class="fas fa-info-circle text-sky-500 text-xl mr-3 mt-0.5"></i> {{-- Icon dan warna diubah --}}
                    </div>
                    <div>
