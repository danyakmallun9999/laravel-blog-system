        ...
        <article class="max-w-3xl mx-auto">
            ...
            <div class="mb-10 sm:mb-12">
                <div class="relative overflow-hidden rounded-2xl bg-gray-100 aspect-[16/10]">
                    ...
                    @else
                        <img src="{{ asset('storage/' . $work->image) }}" alt="{{ $work->title }}" class="w-full h-full object-cover">
                    @endif
                </div>
            </div>
            @endif
            ...
            <nav class="mt-16 pt-8 border-t border-gray-200">
                <div class="flex justify-between items-center gap-4">
                    <!-- Proyek Sebelumnya -->
                    @if($previousWork)
                    <a href="{{ route('work.show', $previousWork) }}" class="flex items-center text-gray-600 hover:text-gray-800 transition w-1/2 pr-2">
                        <svg class="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    ...
                    <!-- Proyek Berikutnya -->
                    @if($nextWork)
                    <a href="{{ route('work.show', $nextWork) }}" class="flex items-center text-gray-600 hover:text-gray-800 transition w-1/2 justify-end pl-2">
                        <div class="text-right overflow-hidden">
                            <div class="text-xs text-gray-500">Next</div>
                            <div class="font-medium truncate">{{ $nextWork->title }}</div>
                        </div>
                        <svg class="w-4 h-4 ml-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    </a>
                    @else
                    <div></div>
                    @endif
