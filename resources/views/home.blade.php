                    ...
                    <article class="bg-white">
                        @if($post->image)
                            @if(Str::startsWith($post->image, 'http'))
                                <img src="{{ $post->image }}" alt="{{ $post->title }}" class="w-full h-48 object-cover rounded-lg mb-6">
                            @else
                                <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}" class="w-full h-48 object-cover rounded-lg mb-6 shadow-md">
                            @endif
                        @endif
                ...
                @if(($featuredWorks ?? collect())->count() > 0)
                    ...
                    <a href="{{ route('work.show', $work) }}" class="group block mb-12 pb-12 border-b border-gray-200 last:border-b-0 last:mb-0 last:pb-0">
                        <div class="flex flex-col md:flex-row gap-8">
                            ...
                            <div class="md:w-2/3">
                                <h3 class="text-2xl font-bold text-gray-900 mb-4 group-hover:text-red-500 transition-colors">{{ $work->title }}</h3>
